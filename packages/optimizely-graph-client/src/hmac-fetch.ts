...
export function createHmacFetch(appKey: string, secret: string): FetchAPI {
  ...
  return async function newFetch(
    input: Parameters<FetchAPI>[0] | URL,
    init?: Parameters<FetchAPI>[1]
  ): ReturnType<FetchAPI> {
    ...
    const body = isRequest(input)
      ? input.body
        ? (await readStream(input.body)).map((x) => new TextDecoder().decode(x)).join("")
        : ""
      : String(init?.body ?? "");
