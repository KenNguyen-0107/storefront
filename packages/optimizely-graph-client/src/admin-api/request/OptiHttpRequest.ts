...
export class OptiHttpRequest extends BaseHttpRequest {
  ...
  public override request<T>(options: ApiRequestOptions): CancelablePromise<T> {
    const fetchApi =
      this.optiGraphConfig?.app_key && this.optiGraphConfig?.secret
        ? createHmacFetch(this.optiGraphConfig.app_key, this.optiGraphConfig.secret)
        : fetch;
...
export function isOptiHttpRequest(toTest: any): toTest is OptiHttpRequest {
  ...
  return (
    typeof (toTest as OptiHttpRequest).request == "function" &&
    typeof (toTest as OptiHttpRequest).setOptiGraphConfig == "function"
  );
}
