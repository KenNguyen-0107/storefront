import {
  ...
  OptimizelyGraphConfigInternal,
} from "../config.js";
import {
  OptimizelyGraphAdminApi as BaseOptimizelyGraphAdminApi,
  type ApiError,
...
export class OptimizelyGraphAdminApi extends BaseOptimizelyGraphAdminApi {
  ...
  public constructor(config?: OptimizelyGraphConfig) {
    ...
    if (isOptiHttpRequest(this.request)) this.request.setOptiGraphConfig(graphConfig);
   ...
   */
  public postSourceContent(
    sourceId: string,
    contentItems: string
  ): CancelablePromise<ClientTypes.PostContentV2DataHandlerResponse> {
