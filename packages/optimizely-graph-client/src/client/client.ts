import {
  ...
  applyConfigDefaults,
  validateConfig,
  type OptimizelyGraphConfigInternal,
...
import {
  ...
  type OptiGraphSiteInfo,
  type IOptiGraphClientFlags,
  type OptiCmsSchema,
  type FrontendUser,
  SchemaVersion,
} from "./types";
import createHmacFetch, { type FetchAPI } from "../hmac-fetch";
import { base64encode, isError, validateToken, getAuthMode, isValidFrontendUser } from "./utils";
...
export class ContentGraphClient extends GraphQLClient implements IOptiGraphClient {
  ...
  protected set token(newValue: string | undefined) {
    if (!validateToken(newValue)) throw new Error("❌ [Optimizely Graph] Invalid ContentGraph token");
    ...
    if (this.debug) console.log(`🔑 [Optimizely Graph] Updating token to ${newValue}`);
  ...
  protected get hmacFetch(): FetchAPI {
    if (!this._hmacFetch) {
      if (this._config.app_key == undefined || this._config.secret == undefined)
        throw new Error(
          "❌ [Optimizely Graph] Unable to create an authenticated connection, make sure both the AppKey & Secret are configured"
        );
  ...
  public constructor(
    ...
    token: AuthMode | string | undefined = undefined,
    ...
    if (!validateToken(token)) throw new Error("❌ [Optimizely Graph] Invalid ContentGraph token");
...
🔖 [Optimizely Graph] [Request Variables] ${JSON.stringify(request.variables)}`);
      ...
      responseMiddleware: (response) => {
        if (isError(response)) {
          ...
          response.errors.forEach(({ message, locations, path, name, source }) => {
            ...
            console.error(
              `❌ [Optimizely Graph] [GraphQL${errorName} error${sourceName}]:\n  Message: ${message}\n  Location: ${locationList}\n  Path: ${path}\n  Query: ${sourceInfo}`
            );
          });
        } else if (QUERY_LOG) {
          console.log(`📦 [Optimizely Graph] [Response Data] ${JSON.stringify(response.data)}`);
          console.log(
            `🔖 [Optimizely Graph] [Response Cost] ${JSON.stringify((response.extensions as { cost?: number } | undefined)?.cost || 0)}`
          );
        }
      },
    });
  ...
  public updateAuthentication(tokenOrAuthmode?: string | AuthMode | undefined): ContentGraphClient {
    ...
    if (tokenOrAuthmode == AuthMode.User)
      throw new Error(
        "❌ [Optimizely Graph] User mode authentication automatically activated when setting user information"
      );
  ...
  public updateFlags(newFlags: Partial<IOptiGraphClientFlags>, temporary: boolean = false): IOptiGraphClient {
    ...
    if (this._oldFlags)
      throw new Error(
        "❌ [Optimizely Graph] There's a temporary flag update in progress, revert that one first prior to updating the flags"
      );
    if (temporary) this._oldFlags = this._flags;
    this._flags = { ...this._flags, ...newFlags };

    if (this.debug)
      console.log(
        `🔵 [Optimizely Graph] ${temporary ? "Temporary updating" : "Updating"} the feature configuration of the client`
      );
  ...
  public restoreFlags(): IOptiGraphClient {
    // Restore the flags
    if (!this._oldFlags) return this;
    this._flags = this._oldFlags;
    this._oldFlags = undefined;

    if (this.debug) console.log(`🔵 [Optimizely Graph] Rolling back the feature configuration of the client`);
  ...
  public setFrontendUser(newUser: FrontendUser | null): boolean {
    ...
    if (!isValidFrontendUser(newUser)) return false;
  ...
  protected updateRequestConfig(): void {
    ...
    if (this.cacheEnabled) this.requestConfig.cache = undefined;
    else this.requestConfig.cache = "no-store";

    // Apply Next.JS Fetch directives
    if (this._flags.nextJsFetchDirectives) {
      const tags: string[] = [
        "opti-graph",
        this.currentAuthMode == AuthMode.Public ? "opti-graph-public" : "opti-graph-private",
      ];
    ...
    if (this.debug) console.log(`🔗 [Optimizely Graph] Updated service endpoint headers: ${JSON.stringify(headers)}`);
  ...
  protected buildDefaultHeaders(): Record<string, string> {
    ...
    const headers: Record<string, string> = {
      "X-Client": "@niteco/OptimizelyGraphClient",
    };

    // Add feature flag based headers
    if (this.graphSchemaVersion == SchemaVersion.Next) headers["cg-query-new"] = "true";
    if (this._flags.includeDeleted) headers["cg-include-deleted"] = "true";
    if (this._flags.recursive) headers["cg-recursive-enabled"] = "true";
  ...
  protected buildUrlWithFlags() {
    ...
    if (this.debug) console.log(`🔗 [Optimizely Graph] Setting service endpoint to: ${serviceUrl.href}`);
