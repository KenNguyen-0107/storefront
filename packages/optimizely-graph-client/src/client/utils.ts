import { OptimizelyGraphConfig } from "../types";
import { AuthMode, type IOptiGraphClient, type FrontendUser } from "./types";
...
export function isValidFrontendUser(toTest?: any): toTest is FrontendUser {
  ...
  if (typeof toTest != "object" || toTest == null || Object.getOwnPropertyNames(toTest).length != 2) return false;

  // FrontendUser has string property `username` and `roles`, each being non-empty
  return (
    typeof toTest?.username == "string" &&
    toTest.username.length > 0 &&
    typeof toTest?.roles == "string" &&
    toTest.roles.length > 0
  );
}
...
export function validateToken(newToken?: string): boolean {
  return (
    newToken == undefined ||
    newToken == AuthMode.HMAC ||
    newToken == AuthMode.Basic ||
...
export function getAuthMode(token?: string): AuthMode {
  switch (token) {
    ...
    default:
      if (typeof token == "string" && token.length > TOKEN_MIN_LENGTH) return AuthMode.Token;
      return AuthMode.Public;
  }
}

export function base64encode(binaryString: string) {
  if (Buffer) return Buffer.from(binaryString).toString("base64");
...
export function isError(toTest: any): toTest is Error {
  return (
    ...
    toTest != null &&
    typeof (toTest as Error).name == "string" &&
    typeof (toTest as Error).message == "string"
  );
}
...
export function isOptiGraphClient(client: any): client is IOptiGraphClient {
  ...
  return (
    typeof (client as IOptiGraphClient).updateAuthentication == "function" &&
...
export function isOptiGraphConfig(client: any): client is OptimizelyGraphConfig {
  if (typeof client != "object" || client == null) return false;
  return (
    typeof (client as OptimizelyGraphConfig).single_key == "string" &&
    (client as OptimizelyGraphConfig).single_key.length > 0
  );
}
