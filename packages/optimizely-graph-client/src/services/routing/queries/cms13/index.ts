import * as GetAllRoutes from "./getAllRoutes";
import * as GetRouteById from "./getRouteById";
import * as GetRouteByPath from "./getRouteByPath";
import type { Route } from "../../types";
import { type IOptiGraphClient as GraphQLClient } from "../../../../client/types";
import { type OptimizelyCmsRoutingApi } from "../types";
...
export class OptimizelyCms13Client implements OptimizelyCmsRoutingApi {
  async getRoutes(client: GraphQLClient, siteId?: string, onlyWithDomain?: boolean): Promise<Route[]> {
    ...
    do {
      ...
      const page = await client
        .request<GetAllRoutes.Result, GetAllRoutes.Variables>(GetAllRoutes.query, {
          domain: siteId,
        ...
        .catch((e) => {
          if (client.debug) console.error("🔴 [RouteResolver] Error while fetching routes", e);
          return undefined;
        });
  ...
  async getRouteByPath(client: GraphQLClient, path: string, siteId?: string): Promise<undefined | Route> {
    ...
    if ((resultSet.getRouteByPath?.total ?? 0) === 0) {
      if (client.debug) console.warn("🟠 [RouteResolver] No items in the resultset");
      return undefined;
    }
    ...
    if (client.debug)
      console.log(
        `⚪ [RouteResolver] Resolved content info for ${path} to: ${JSON.stringify(resultSet.getRouteByPath.items)}`
      );
  ...
  async getRouteById(
    ...
    if (client.debug) console.log("Resolving content by id:", JSON.stringify(variables));
    ...
    if (resultSet.Content?.total >= 1) {
      if (client.debug && resultSet.Content?.total > 1)
        console.warn(
          `Received multiple entries with this ID, returning the first one from: ${(resultSet.Content?.items || [])
            .map((x) => {
              return `${x._metadata.key} (version: ${x._metadata.version}, locale: ${x._metadata.locale})`;
            })
            .join("; ")}`
        );
  ...
  private convertResponse(item: GetAllRoutes.Route): Route {
    if (!item) throw new Error('RouteResolver.convertResponse(): mandatory parameter "item" not provided!');
