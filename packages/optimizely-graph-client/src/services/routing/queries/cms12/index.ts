...
export class OptimizelyCms12Client implements OptimizelyCmsRoutingApi {
  async getRoutes(client: GraphQLClient, siteId?: string): Promise<Route[]> {
    ...
    let page = await client.request<GetAllRoutes.Result, GetAllRoutes.Variables>(GetAllRoutes.query, {
      siteId,
      typeFilter: "Page",
    });
  ...
  async getRouteByPath(client: GraphQLClient, path: string, siteId?: string): Promise<undefined | Route> {
    if (client.debug) console.log(`Resolving content info for ${path} on ${siteId ? "site " + siteId : "all sites"}`);
    ...
    if (client.debug) console.log(`Resolved content info for ${path} to:`, resultSet.Content.items[0]);
      ...
      if (client.debug && resultSet.Content?.total > 1)
        console.warn(
          `Received multiple entries with this ID, returning the first one from: ${(resultSet.Content?.items || [])
            .map((x) => {
              return `${x.contentLink.id}_${x.contentLink.workId}_${x.locale.name}`;
            })
            .join("; ")}`
        );
      return this.convertResponse(resultSet.Content.items[0]);
