...
import type { OptimizelyCmsRoutingApi, OptimizelyCmsRoutingApiClass } from "./queries/types";
  ...
  private async getResolver(): Promise<OptimizelyCmsRoutingApi> {
    if (!this._resolver) {
      ...
      if (this._resolverMode == OptiCmsSchema.CMS12) {
        ResolverClass = (await import("./queries/cms12/index")).default;
      } else {
        ResolverClass = (await import("./queries/cms13/index")).default;
      }
