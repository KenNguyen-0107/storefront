import type { ChannelDomain, ChannelLocale } from "./types";
import { type OptimizelyGraphConfig } from "../../types";
import createClient, { isOptiGraphClient, type IOptiGraphClient, OptiCmsSchema } from "../../client/index";
import ChannelDefinition from "./definition";
import * as Queries from "./queries";
import { localeToGraphLocale } from "../utils";
...
export class ChannelRepository {
  ...
  public async getAll(): Promise<ReadonlyArray<Readonly<ChannelDefinition>>> {
    ...
    if (!channels || !Array.isArray(channels)) throw new Error("No channels returned by Optimizely Graph");
  ...
  public async getByDomain(domain: string, fallback: boolean = true): Promise<Readonly<ChannelDefinition> | null> {
    const data = await this.client.request<{ GetChannelByDomain?: { channels: {}[] } }>(Queries.getByDomain, {
      domain,
      fallback: fallback ? "*" : "__NO_FALLBACK__",
    });
    ...
    return new ChannelDefinition(
      {
        ...
        domains: (ch.domains ?? []).map((d: any) => {
          ...
          if (d.forLocale?.code) def.forLocale = d.forLocale?.code;
          return def;
        }),
