import type {
  ReadonlyChannelDefinitionData,
  ChannelDefinitionData,
  ChannelContent,
  ChannelDomain,
  ChannelLocale,
} from "./types";
...
export class ChannelDefinition implements Readonly<ChannelDefinitionData> {
  ...
  public get defaultDomain(): string {
    const primaryDomain: ChannelDomain | undefined = this.isDev
      ? this.domains.filter((x) => x.name.includes("localhost"))[0]
      : this.domains.filter((x) => x.isPrimary)[0];
  ...
  public resolveLocale(code?: string): string {
    return code
      ? (this.locales.filter((locale) => locale.code == code)[0]?.code ?? this.defaultLocale)
      : this.defaultLocale;
  }
