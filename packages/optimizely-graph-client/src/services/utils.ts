import type { ContentLink, ContentLinkWithLocale, InlineContentLink, InlineContentLinkWithLocale } from "./types";
...
export function graphLocaleToLocale<T>(graphLocale: T): T extends string ? string : never {
  if (
    typeof graphLocale != "string" ||
    (graphLocale.length != 2 && graphLocale.length != 5 && graphLocale != "ALL" && graphLocale != "NEUTRAL")
  )
 ...
 */
export function contentLinkIsEqual(
  link1: ContentLink | ContentLinkWithLocale,
  link2: ContentLink | ContentLinkWithLocale
): boolean {
  if (link1.key != link2.key) return false;
  if ((link1.version || link2.version) && link1.version != link2.version) return false;
  if (
    ((link1 as ContentLinkWithLocale).locale || (link2 as ContentLinkWithLocale).locale) &&
    (link1 as ContentLinkWithLocale).locale != (link2 as ContentLinkWithLocale).locale
  )
    return false;
  return true;
 ...
 * @returns     True if they links point to the same content item, False otherwise
 */
export function contentLinkIsEqualIgnoreVersion(
  link1: ContentLink | ContentLinkWithLocale,
  link2: ContentLink | ContentLinkWithLocale
): boolean {
  if (link1.key != link2.key) return false;
...
export function isContentLinkWithLocale(toTest: any): toTest is ContentLinkWithLocale {
  if (!isContentLink(toTest)) return false;

  const locale = (toTest as ContentLinkWithLocale).locale;
  return (
    locale == undefined || locale == null || (typeof locale == "string" && locale.length >= 2 && locale.length <= 5)
  );
}
 ...
 * @returns
 */
export function normalizeContentLink(
  toNormalize: Nullable<ContentLink | InlineContentLink>
): ContentLink | InlineContentLink | undefined {
  if (!(isContentLink(toNormalize) || isInlineContentLink(toNormalize))) return undefined;

  const newLink: ContentLink | InlineContentLink = {
    key: toNormalize.key,
  };
  if (toNormalize.version) newLink.version = toNormalize.version.toString();

  return toNormalize;
}

export function normalizeContentLinkWithLocale<LT = string>(
  toNormalize: Nullable<ContentLinkWithLocale<LT>>
): ContentLinkWithLocale<LT> | undefined {
  const normalized = normalizeContentLink(toNormalize) as ContentLinkWithLocale<LT>;
  if (
    toNormalize?.locale != undefined &&
    !(typeof toNormalize?.locale == "string" && (toNormalize.locale.length == 2 || toNormalize.locale.length == 5))
  )
 ...
 */
export function contentLinkToString(
  contentLink: ContentLink | InlineContentLink | null | undefined,
  unique: boolean = false
): string {
  if (!contentLink) return unique ? generateUniqueKey("no-content::") : "[no content]";
  return [
