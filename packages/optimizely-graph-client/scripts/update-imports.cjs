...
function processDirectory(apiPath, basePath) {
  ...
  while ((entry = directory.readSync())) {
    ...
    if (entry.isFile() && shouldProcessFile(entry.name)) promises.push(processFile(entryPath, basePath));
    else if (entry.isDirectory()) promises.push(processDirectory(entryPath, basePath));
  }
...
function processFile(filePath, basePath) {
  return new Promise((resolve, reject) => {
    fs.readFile(filePath, (err, data) => {
      ...
      const replaced = currentData.replaceAll(
        /^(?:import|export(?:\ type){0,1}\ \{).*from +[\'\"](.*)[\'\"];{0,1}$/gm,
        (match, file) => {
