...
import { useState, useEffect, useRef, type FunctionComponent, type PropsWithChildren } from "react";
...
export const OnPageEdit: FunctionComponent<PropsWithChildren<{ refreshTimeout?: number | false }>> = ({
  children,
  ...
  function onContentSaved(eventData: OptimizelyCmsContentSavedEvent) {
    ...
    if (eventData?.previewUrl && previewUrl != eventData.previewUrl) {
      ...
      if (refreshTimeout) reloadDelay.current = setTimeout(() => router.refresh(), refreshTimeout);
      else router.refresh();
    }
    setShowMask(false);
  }
