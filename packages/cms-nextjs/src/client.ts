import "server-only";
import createBaseClient, { type IOptiGraphClient, AuthMode } from "@niteco/optimizely-graph-client";

export const getServerClient: () => IOptiGraphClient = () => {
  const client = createBaseClient();
  if (client.debug) console.log("⚪ [ContentGraph Shared Client] Created new Optimizely Graph client");
  return client;
};
...
export const getAuthorizedServerClient: (token?: string) => IOptiGraphClient = (token) => {
  ...
  if (typeof token == "string" && token.length > 0) {
    ...
    } else client.updateAuthentication(token);
  }
  if (client.debug)
