...
export type PublishApiHandler = (
  req: NextRequest
) => Promise<NextResponse<PublishApiResponse>> | NextResponse<PublishApiResponse>;
...
type PartialRouteResolver = {
  getContentInfoById: (
    ...args: Parameters<InstanceType<typeof RouteResolver>["getContentInfoById"]>
  ) => Promise<undefined | Pick<Route, "path">>;
};
...
export type PublishApiOptions = {
   ...
   */
  tags:
    | Parameters<typeof revalidateTag>[0]
    | Array<Parameters<typeof revalidateTag>[0]>
    | ((data: PublishHookData | null | undefined) => Promise<Array<Parameters<typeof revalidateTag>[0]>>);
  ...
  itemIdToKeyAndLocale: (
    itemId: string,
    subject?: PublishHookData["type"]["subject"]
  ) => { key: string; locale: string; version?: string; status?: string } | undefined;
...
const publishApiDefaults: PublishApiOptions = {
  ...
  itemIdToKeyAndLocale: (id) => {
    ...
    const [key, version, locale, status] =
      idParts.length >= 4 ? idParts : [idParts[0], undefined, idParts[1], idParts[2]];

    // Only return a value if we have both a key & locale
    if (key && locale) return { key: key.replaceAll("-", ""), locale, version, status };
    return undefined;
  },
};
...
export function createPublishApi(options?: Partial<PublishApiOptions>): PublishApiHandler {
  ...
  function getRouteResolver(client: IOptiGraphClient): PartialRouteResolver {
    return typeof routerFactory == "function"
      ? routerFactory()
      : new RouteResolver(client, routerFactory?.urlBase, routerFactory?.resolverMode ?? client.currentOptiCmsSchema);
  }

  function resolveTags(data: PublishHookData | null | undefined): Promise<Array<Parameters<typeof revalidateTag>[0]>> {
    if (typeof tags == "function") return tags(data);
    if (Array.isArray(tags)) return Promise.resolve(tags);
  ...
  function publishAll(
    targetPaths: string[],
    targetTags: Array<Parameters<typeof revalidateTag>[0]>,
    optimized: boolean = false,
    publishScopes: PublishScopes[] = scopes,
    dscopes: DynamicScopes[] = dynamicScopes
  ): PublishApiResponse {
  ...
  function getItemIds(hookData: PublishHookData): Array<{ key: string; locale: string }> {
    ...
    switch (hookData.type.subject) {
      case "doc":
        items = [hookData.data.docId].map((item) => itemIdToKeyAndLocale(item)).filter((x) => x) as Array<{
          key: string;
          locale: string;
        }>;
        break;
      case "bulk":
        items = Object.getOwnPropertyNames(hookData.data.items ?? {})
          .map((pn) => {
            const status = hookData.data.items[pn];
            if (bulkItemStatusFilter(status)) return itemIdToKeyAndLocale(pn);
            return undefined;
          })
  ...
  const requestHandler: PublishApiHandler = async (req) => {
    ...
    try {
      ...
      const results = await Promise.allSettled(
        contentIds.map((contentId) => router.getContentInfoById(contentId.key, contentId.locale))
      );
      const pathsToFlush = results
        .map((result) => {
          if (result.status == "rejected") {
            console.error(
              "[Publish-API] Error fetching route for content item:",
...
function getRequestToken(req: NextRequest): string | undefined {
  return req.headers.get("X-OPTLY-PUBLISH") ?? req.nextUrl.searchParams.get("token") ?? undefined;
}
