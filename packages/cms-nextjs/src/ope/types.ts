...
import { type ClientFactory } from "@niteco/optimizely-graph-client";
import { type GraphQLClient } from "graphql-request";
import { type ContentQueryProps } from "@/packages/optimizely-cms-react/src/types";

export type ContentRequest = Omit<ContentQueryProps<string>, "path"> & {
  path: string | null;
  token: string;
  ctx: "edit" | "preview";
};
...
export type EditPageComponent<T extends ServerPageProps = EditPageProps> = ({
  params,
  searchParams,
}: T) => Promise<React.JSX.Element>;
...
export type EditViewOptions<LocaleType = string> = {
  ...
  ) => props is ValidatedEditPageProps;
...
export type GetContentByIdMethod<LocaleType = string> = (
  client: GraphQLClient,
  variables: ContentQueryProps<LocaleType>
) => Promise<GetContentByIdData>;
