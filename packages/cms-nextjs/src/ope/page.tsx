...
import { contentLinkToString, localeToGraphLocale } from "@niteco/optimizely-graph-client/services";
import { type ContentLinkWithLocale } from "@niteco/optimizely-graph-client";
import {
  ServerContext,
  Utils,
  CmsContent,
...
} from "@/packages/optimizely-cms-react/src/rsc";
...
import { getContentRequest, isValidRequest } from "./tools";
...
export function createEditPageComponent<LocaleType = string>(
  ...
  async function EditPage(props: EditPageProps): Promise<React.JSX.Element> {
    ...
    try {
      ...
      const contentItem =
        (Array.isArray(contentInfo?.content?.items) ? contentInfo?.content?.items[0] : contentInfo?.content?.items) ??
        undefined;
        ...
        <>
          ...
          {context.isDebug && (
            <div className="optly-contentLink">
              ContentItem:{" "}
              {contentLink ? contentLinkToString(contentLink) : "Invalid content link returned from Optimizely Graph"}
            </div>
          )}
        </>
      );
