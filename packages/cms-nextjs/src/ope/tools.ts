import type { EditPageProps, ValidatedEditPageProps, ContentRequest } from "./types";
import { AuthMode } from "@niteco/optimizely-graph-client";

export function isValidRequest(
  toTest: EditPageProps,
  throwOnInvalid: boolean = false,
  isDevelopment: boolean = false
): toTest is ValidatedEditPageProps {
  ...
  if (toTest.params.lang && typeof toTest.params.lang != "string") {
    if (throwOnInvalid) throw new Error("🔴 The 'lang' route parameter must be a string");
    return false;
  }
  if (
    toTest.params.path &&
    (!Array.isArray(toTest.params.path) || !toTest.params.path.every((x) => typeof x == "string"))
  ) {
    if (throwOnInvalid) throw new Error("🔴 The 'path' route parameter must be an array of strings");
    return false;
  }
  ...
  if (token.length < 20 && !validDevToken) {
    if (throwOnInvalid) throw new Error('🔴 The search paramater "preview_token" is invalid or missing');
    return false;
  }

  // Validate CTX
  if (
    !(toTest.searchParams.ctx == undefined || toTest.searchParams.ctx == "edit" || toTest.searchParams.ctx == "preview")
  ) {
    if (throwOnInvalid) throw new Error("The 'ctx' search parameter must be omitted or 'edit' or 'preview'");
    return false;
  }
  ...
  if (
    !(
      toTest.searchParams.epieditmode == undefined ||
      toTest.searchParams.epieditmode.toLowerCase() == "true" ||
      toTest.searchParams.epieditmode.toLowerCase() == "false"
    )
  ) {
    if (throwOnInvalid) throw new Error("The 'epieditmode' search parameter must be omitted or 'true' or 'false'");
    return false;
  }

  if (toTest.searchParams.ctx && toTest.searchParams.epieditmode) {
    if (throwOnInvalid) throw new Error("The 'epieditmode' and 'ctx' search parameters cannot be both defined");
    return false;
  }

  if (!toTest.searchParams.ctx && !toTest.searchParams.epieditmode) {
    if (throwOnInvalid) throw new Error("Either 'epieditmode' and 'ctx' search parameters must be defined");
    return false;
  }
...
export function getContentRequest({
  params: { path, lang },
  searchParams: { preview_token: token, ...searchParams },
}: ValidatedEditPageProps): ContentRequest | undefined {
  try {
    ...
    if (token == AuthMode.HMAC || token == AuthMode.Basic) {
      console.error(
        "🦺 [OnPageEdit] Edit mode requested with developer tokens, falling back to URL parsing to determine content"
      );
      // Determine the contentPath
      const fullPath = Array.isArray(path)
        ? path.map((p) => decodeURIComponent(p)).join("/")
        : decodeURIComponent(path ?? "");
      const [cmsPath, contentString] = fullPath.split(",,", 3);
      const contentPath = (cmsPath.startsWith("/") ? cmsPath.substring(1) : cmsPath).replace(
        /^(ui\/){0,1}(CMS\/){0,1}(Content\/){0,1}/gi,
        ""
      );
...
export function readValue<T extends string | undefined>(
  variableName: string,
  defaultValue?: T
...
export function readValueAsInt<T extends number | undefined>(
  variableName: string,
  defaultValue?: T
): T extends number ? number : number | undefined {
  const stringValue = readValue(variableName);
  if (!stringValue) return defaultValue as T extends number ? number : undefined;
  try {
