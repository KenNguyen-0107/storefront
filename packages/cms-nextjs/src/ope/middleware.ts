 ...
 */
export function withEditFallback<T = unknown>(
  middleware?: Middleware<T>,
  adminSlug: string = "ui",
  previewRoute: string = "/preview"
): Middleware<T> {
  ...
  const wrappedMiddleware: Middleware<T> = (request) => {
    if (
      ...
      const token =
        ...
        process.env.OPTIMIZELY_PUBLISH_TOKEN ||
        "use-hmac";
      let requestPath = path.replace(previewPath, "/");
      if (!requestPath.endsWith("/")) requestPath = requestPath + "/";
