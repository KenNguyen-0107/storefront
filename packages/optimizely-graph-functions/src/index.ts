 ...
 * @param pluginContext
 */
export const validate: PluginValidateFn<PluginOptions> = (
  schema,
  document,
  config,
  outputFile,
  ...
  if (config.functions) {
    if (!Array.isArray(config.functions)) throw new Error("If provided functions must be an array");
...
export const plugin: PluginFunction<PluginOptions> = async (schema, documents, config, info) => {
  ...
  if (functions.length == 0) return "// NO FUNCTIONS TO BE EXPORTED";
  ...
  const output = functions
    .map((fn) => {
      try {
        ...
        functionBody.push(
          `export function ${fn}(client: GraphQLClient, variables: ${varsType}) : Promise<${returnType}>`
        );
        functionBody.push("{");
        functionBody.push(
          `  const query = gql\`${config.prettyPrintQuery ? query : query.replace(/\s+/g, " ").trim()}\``
        );
...
function resolveSpreads(
  ...
  visit(definition, {
    FragmentSpread: {
      leave(node) {
        if (!availableFragments.includes(node.name.value)) spreadNames.push(node.name.value);
      },
    },
  });
  ...
  visit(document, {
    FragmentDefinition: {
      leave(node) {
        if (spreadNames.includes(node.name.value)) fragments.push(node);
      },
    },
  });
