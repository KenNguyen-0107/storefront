...
export const transform: Types.DocumentTransformFunction<TransformOptions> = ({
  documents: files,
  config,
  schema,
  pluginContext,
}) => {
  if (config.verbose) console.debug(`[ OPTIMIZELY ] Starting Optimizely Graph Query & Fragment transformations`);
  const injections = config.injections ?? [];

  // Retrieve component fragments
  if (config.verbose) console.debug(`[ OPTIMIZELY ] Searching for fragments to inject`);
  const componentFragments: { [into: string]: FragmentDefinitionNode[] } = {};
  files.forEach((file) => {
    if (!file.document) return;
    const applicableInjections = injections.filter(
      (injection) => !injection.pathRegex || new RegExp(injection.pathRegex).test(file.location ?? "")
    );
    if (!applicableInjections || applicableInjections.length == 0) return;
    visit(file.document, {
      FragmentDefinition: {
        enter(node) {
          const matchingInjections = applicableInjections.filter(
            (injection) => !injection.nameRegex || new RegExp(injection.nameRegex).test(node.name.value)
          );
          if (!matchingInjections || matchingInjections.length == 0) return false;
          matchingInjections.forEach((injection) => {
            if (config.verbose)
              console.debug(
                `[ OPTIMIZELY ] Found ${node.name.value} for ${injection.into} in file ${node.loc?.source?.name}`
              );
            if (!componentFragments[injection.into]) componentFragments[injection.into] = [];
  ...
  const transformedFiles = files.map((file) => {
    if (config.verbose) console.debug(`[ OPTIMIZELY ] Processing ${file.location}`);
          ...
          SelectionSet: {
            leave(node, key, parent, path, ancestors) {
              ...
              const sectionsToAdd = node.selections
                .map((selection) => {
                  if (selection.kind != Kind.FRAGMENT_SPREAD) return undefined;
                  const testableName =
                    config.recursion && selection.name.value.startsWith("Recursive")
                      ? selection.name.value.substring(9)
                      : selection.name.value;
                  if (config.recursion && config.verbose && testableName != selection.name.value)
                    console.debug(
                      `[ OPTIMIZELY ] Using ${selection.name.value} in ${parentName} as ${testableName} to allow recursion`
                    );
              ...
              if (config.verbose)
                console.debug(
                  `[ OPTIMIZELY ] Identified usage of fragment(s) ${sectionsToAdd.join(", ")} in ${parentName}, starting injection procedure`
                );
              ...
              sectionsToAdd.forEach((sectionName) => {
                const addedSelections: FragmentSpreadNode[] = componentFragments[sectionName]
                  .map((fragment) => {
                    if (
                      newSelections.some(
                        (selection) =>
                          selection.kind == Kind.FRAGMENT_SPREAD && selection.name.value == fragment.name.value
                      )
                    ) {
                      if (config.verbose)
                        console.debug(
                          `[ OPTIMIZELY ] Fragment ${fragment.name.value} is already adjacent to ${sectionName}`
                        );
                      return undefined;
                    }
                    /*if (config.verbose) 
                ...
                if (addedSelections.length > 0) {
                  if (config.verbose)
                    console.log(
                      `[ OPTIMIZELY ] Added fragments ${addedSelections.map((a) => a.name.value).join(", ")} adjacent to ${sectionName}`
                    );
  ...
  if (config.verbose) console.debug(`[ OPTIMIZELY ] Finished transformation procedure`);
