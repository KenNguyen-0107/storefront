...
export const preset: Types.OutputPreset<PresetOptions> = {
  ...
  prepareDocuments: async (
    ...
    const optiDocs = outputSpecificDocuments.filter<string>(
      ((x) => typeof x == "string" && x.startsWith("opti-cms:")) as (x: Types.OperationDocument) => x is string
    );
    ...
    const CmsDocLoaders: Array<Types.CustomDocumentLoader> = (
      optiDocs.length == 0 ? ["opti-cms:/fragments/13", "opti-cms:/queries/13"] : optiDocs
    ).map((optiDoc) => {
      const loader: Types.CustomDocumentLoader = {};
      loader[optiDoc] = { loader: "./packages/optimizely-graph-functions/dist/embedded-loader.js" };
      return loader;
    });
      ...
      if (doc.document) {
        const newDocument = visit(doc.document, {
          FragmentDefinition: {
            enter: (node) => {
              if (
                !options.schema.definitions.some(
                  (x) =>
                    (x.kind == Kind.OBJECT_TYPE_DEFINITION || x.kind == Kind.INTERFACE_TYPE_DEFINITION) &&
                    x.name.value == node.typeCondition.name.value
                )
              ) {
                if (options.presetConfig.verbose)
                  console.log(
                    `⚠ Removing fragment ${node.name.value} from the documents, as its target ${node.typeCondition.name.value} is not available in the schema`
                  );
                return null;
              }
            },
