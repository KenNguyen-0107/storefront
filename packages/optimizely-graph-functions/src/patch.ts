...
const downloadLink =
  "https://github.com/remkoj/optimizely-dxp-clients/raw/refs/heads/main/dependencies/graphql-codegen-visitor-plugin-common-v5.7.2-patched.tgz";
...
fetch(downloadLink).then((response) => {
  ...
  finished(
    Readable.fromWeb(response.body as Parameters<(typeof Readable)["fromWeb"]>[0]).pipe(
      fs.createWriteStream(downloadTarget, { flags: "w" })
    )
  ).then(() => {
