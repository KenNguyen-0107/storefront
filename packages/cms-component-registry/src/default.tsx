...
export class DefaultComponentFactory implements ComponentFactory {
  ...
  resolve(type: ComponentTypeHandle): undefined | ComponentType {
    ...
    const ErrorBoundary = entry.errorBoundary || DefaultErrorBoundary;

    function Suspended(props: Record<string, unknown>) {
      return (
        <ErrorBoundary>
...
function processComponentTypeHandle(handle: ComponentTypeHandle): string {
  if (typeof handle == "string") return handle == "" ? EmptyComponentHandle : handle;
  ...
  throw new Error(`Invalid component type handle: ${typeof handle}::${JSON.stringify(handle)}`);
}
