 ...
 */
export function isValidUrl(toTest: unknown): boolean {
  if (!isNonEmptyString(toTest)) return false;
  try {
 ...
 */
export function isValidHttpsUrl(toTest: unknown): boolean {
  if (!isNonEmptyString(toTest)) return false;
  try {
    const u = new URL(toTest);
 ...
 */
export function getBoolean(envVarName: string, defaultValue: boolean | (() => boolean) = false): boolean {
 ...
 */
export function getMandatory(envVarName: string): string {
 ...
 */
export function getSelection<T>(envVarName: string, allowedValues: T[], defaultValue: T): T {
 ...
 */
export function ensureHttpsProtocol(url: string | undefined): string | undefined {
