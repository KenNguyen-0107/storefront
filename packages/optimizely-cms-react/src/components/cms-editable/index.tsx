...
import { GenericContext, PropsWithContext } from "../../context/types";
import type { ElementType, GenericContextProps, ElementProps } from "../type-utils";
import { type ContentLink } from "@niteco/optimizely-graph-client";
...
export const CmsEditable: CmsEditableBaseComponent = <CT extends ElementType>({
  ...
  const addEditProps = inEditMode ? (currentContent ? editableContent?.key == currentContent.key : true) : false;
  ...
  const dataEpiPropertyName = editableContentIsExperience ? undefined : (cmsFieldName ?? undefined);
  ...
  if (typeof forwardCtx == "string" && forwardCtx.length > 0) itemProps[forwardCtx] = ctx;
