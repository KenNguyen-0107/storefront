...
import { normalizeContentLinkWithLocale, contentLinkToString } from "@niteco/optimizely-graph-client/services/utils";
import type {
...
>({
  ...
  fallback,
  noWrapper: noContentAreaContainer = false as BaseCmsContentAreaProps<T, I>["noWrapper"],
  variant,
  ...
  const componentData = ((items || []) as Array<ContentAreaItemDefinition | null | undefined>)
    ...
    .map((item, idx) => {
      ...
      const contentAraeItemContent: React.JSX.Element = useSuspense ? (
      ...
      if (childrenTarget == "children") contentAreaItemContainerChildren = contentAraeItemContent;
      else contentAreaItemContainerProps[childrenTarget] = contentAraeItemContent;

      return (
        <ContentAreaItemContainer key={contentAreaItemKey} {...contentAreaItemContainerProps}>
  ...
  if (contentAreaContainerChildrenTarget == "children") contentAreaContainerChildren = componentData;
  else contentAreaContainerProps[contentAreaContainerChildrenTarget] = componentData;
