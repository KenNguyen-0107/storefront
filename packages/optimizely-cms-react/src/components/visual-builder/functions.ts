import { isContentType } from "../../utilities";
import { isContentLink, ContentLinkWithLocale, isInlineContentLink } from "@niteco/optimizely-graph-client";
import type {
  CompositionNode,
  LeafPropsFactory,
...
): node is CompositionComponentNode<Record<string, any>> {
...
export function isComponentNodeOfType<ET extends Record<string, any>>(
  node: CompositionNode<Record<string, any>>,
  test: (data: Record<string, any>) => data is ET
): node is CompositionComponentNode<ET> {
...
export function isNode(toTest: any): toTest is CompositionNode {
  ...
  const nodeTypes = ["experience", "section", "row", "column", "component", "outline"];
  const hasValidName =
    (typeof (toTest as CompositionNode).name == "string" && ((toTest as CompositionNode).name?.length ?? 0) > 0) ||
    (toTest as CompositionNode).name == null;
  const hasValidType =
    typeof (toTest as CompositionNode).layoutType == "string" &&
    nodeTypes.includes((toTest as CompositionNode).layoutType);

  return hasValidName && hasValidType;
}

export const defaultPropsFactory: LeafPropsFactory = <ET extends Record<string, any>, LT = string>(
  node: CompositionComponentNode<ET>
) => {
  const contentType = node.component?._metadata?.types;
  if (!isContentType(contentType)) throw new Error("Invalid content type: " + JSON.stringify(contentType));
...
export const defaultNodePropsFactory: NodePropsFactory = <ET extends Record<string, any>, LT = string>(
  node: CompositionStructureNode
) => {
  const componentTypes = [
    // New style logic
    node.template &&
      ([node.template, ucFirst(node.type), ucFirst(node.layoutType), "Content"].filter((x) => x) as string[]),
    ...
    node.template && node.type
      ? ([node.type ? ucFirst(node.type) : null, ucFirst(node.layoutType), "Component", "Content"].filter(
          (x) => x
        ) as string[])
      : null,
  ...
  if (!(isContentLink(contentLink) || isInlineContentLink(contentLink)))
    throw new Error(
      "🔴 [VisualBuilder] Invalid content link: " + JSON.stringify(contentLink) + " - Node: " + JSON.stringify(node)
    );
...
export function ucFirst(input: string | undefined | null): string | null {
  if (typeof input == "string" && input.length > 0) return input[0].toUpperCase() + input.substring(1);
  return null;
}
