...
import { isComponentNode, defaultNodePropsFactory, defaultPropsFactory } from "./functions";
import type { BaseOptimizelyCompositionProps } from "./types";
import { link } from "fs";
...
export function OptimizelyComposition({
  ...
  if (isComponentNode(node)) {
    if (isDebug) console.log(`⚪ [VisualBuilder] Rendering element node ${JSON.stringify(node)}`);
  ...
  // Debug
  if (isDebug) console.log(`⚪ [VisualBuilder] Rendering structure node ${JSON.stringify(node)}`);
