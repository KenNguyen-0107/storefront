...
import {
  type ContentLink,
  type InlineContentLink,
  type IOptiGraphClient,
  isContentLink,
  isInlineContentLink,
  OptiCmsSchema,
} from "@niteco/optimizely-graph-client";
import { CmsComponent, CmsComponentWithFragment } from "../../types";
import { CmsContentFragments } from "../../data/queries";
import {
  validatesFragment,
  contentLinkToRequestVariables,
  isCmsComponentWithFragment,
  isCmsComponentWithDataQuery,
} from "../../utilities";

export function getContent<NDL extends boolean = false>(
  client: IOptiGraphClient | undefined,
  contentLink: InlineContentLink | ContentLink | undefined,
  Component: CmsComponent<any> | undefined,
  fragmentData: Record<string, any> | undefined | null,
  noDataLoad?: NDL
): NDL extends true ? Record<string, any> : Promise<Record<string, any>> {
  ...
  const fragmentProps = Object.getOwnPropertyNames(myFragmentData).filter(
    (x) => !CmsContentFragments.IContentDataProps.includes(x)
  );
  if (fragmentProps.length > 0) {
    if (validatesFragment(Component) && !Component.validateFragment(myFragmentData))
      console.warn(
        "🔴 [CmsContent][getContent] Invalid fragment data received, falling back to loading for ",
        componentLabel
      );
    else {
      if (debug) console.log("⚪ [CmsContent][getContent] Rendering CMS Component using fragment information");
      return (noDataLoad ? myFragmentData : Promise.resolve(myFragmentData)) as NDL extends true
        ? Record<string, any>
  ...
  if (isInlineContentLink(contentLink)) {
    if (debug)
      console.warn(
        `🟠 [CmsContent][getContent] No data present for inline content item, this may cause errors if your component expects certain fields to be present`
      );
    return (noDataLoad ? myFragmentData : Promise.resolve(myFragmentData)) as NDL extends true
      ? Record<string, any>
      : Promise<Record<string, any>>;
  ...
  if (!isContentLink(contentLink)) {
    if (debug)
      console.warn(
        `🟠 [CmsContent][getContent] Unable to load data for "${componentLabel}" without a valid content link`
      );
  ...
  if (isCmsComponentWithFragment<any>(Component))
    return getComponentDataFromFragment<any>(Component, contentLink, client).then(
...
async function getComponentDataFromFragment<T extends any = any>(
  Component: CmsComponentWithFragment<T, Record<string, any>>,
  contentLink: ContentLink,
  client: IOptiGraphClient
) {
  type FragmentQueryResponse = {
    contentById: {
      total: number;
      items: Array<
        { _metadata: { key: string; version: number | string; locale?: string }; _locale?: { name: string } } & T
      >;
    };
  };
  ...
  if (client.currentOptiCmsSchema == OptiCmsSchema.CMS12) {
    try {
      ...
      if (versionNr > 0) fragmentVariables.version = versionNr as unknown as string;
      else fragmentVariables.version = undefined;
    } catch {
  ...
  if (totalItems < 1)
    throw new Error(
      `CmsContent expected to load exactly one content item of type ${name}, received ${totalItems} from Optimizely Graph. Content Item: ${JSON.stringify(fragmentVariables)}`
    );
  ...
  if (client.currentOptiCmsSchema == OptiCmsSchema.CMS12) {
    fragmentResponse.contentById.items[0]._metadata.locale =
      fragmentResponse.contentById.items[0]._metadata.locale ?? fragmentResponse.contentById.items[0]._locale?.name;
    if (fragmentResponse.contentById.items[0]._locale) delete fragmentResponse.contentById.items[0]._locale;
  }
