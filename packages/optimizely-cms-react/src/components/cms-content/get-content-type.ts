...
import { contentLinkToRequestVariables, normalizeContentType } from "../../utilities";

export type ContentTypeResolver = (
  type: ContentType | null | undefined,
  link: ContentLink,
  gqlClient: IOptiGraphClient
) => PromiseLike<ContentType | undefined>;
...
export function valueToPromiseLike<T>(value: T): PromiseLike<T> {
  const pl: PromiseLike<T> = {
    then: <TResult1 = T>(onfulfilled?: ((value: T) => TResult1 | PromiseLike<TResult1>) | null | undefined) => {
      if (!onfulfilled) return value as unknown as PromiseLike<TResult1>;
...
export async function getContentType(
  link: ContentLink | InlineContentLink,
  gqlClient: IOptiGraphClient
): Promise<ContentType | undefined> {
  if (isInlineContentLink(link)) {
    console.error(
      `🔴 [CmsContent][getContentType] Unable to dynamically resolve the content type for inline content items: ${contentLinkToString(link)}`
    );
    throw new Error(
      `Unable to dynamically resolve the content type for inline content items: ${contentLinkToString(link)}`
    );
  }

  if (!isContentLink(link)) {
    console.error(
      `🔴 [CmsContent][getContentType] The provided link is not a valid content link: ${contentLinkToString(link)}`
    );
  ...
  if (gqlResponse.Content?.total != 1) {
    if (gqlClient.debug)
      console.error(
        `🔴 [CmsContent][getContentType] Expected exactly one type, received ${gqlResponse.Content?.total ?? 0} types for`,
        gqlQueryVars
      );
    return undefined;
  }
  ...
  if (!items || items.length == 0) {
    console.error(
      `🔴 [CmsContent][getContentType] The content item could not be found! (${contentLinkToString(link)})`
    );
  ...
  if (!contentType) {
    console.error(
      `🔴 [CmsContent][getContentType] The item did not contain type information! (${contentLinkToString(link)})`
    );
...
const getContentTypeQuery = gql`
  query getContentType($key: String!, $version: String, $locale: [Locales]) {
    Content: _Content(
