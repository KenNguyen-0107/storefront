...
import { type GenericContext, type RenderMode, type TransferrableContext } from "./types";
import { type ComponentFactory, type ComponentTypeDictionary, DefaultComponentFactory } from "../factory/index";
import { isDebug, isDevelopment } from "../rsc-utilities";
...
export class ServerContext implements GenericContext {
  ...
  public constructor({ factory, client, locale, mode, editableContent }: ServerContextArgs) {
  ...
  public setMode(mode: "edit" | "preview" | "public"): ServerContext {
    if (this.isDebug) console.log(`🦺 [ServerContext] Updating mode from ${this._mode} to ${mode}`);
    this._mode = mode;
    return this;
  }

  public setLocale(locale: string): ServerContext {
    if (this.isDebug) console.log(`🦺 [ServerContext] Updating locale from ${this._locale} to ${locale}`);
    this._locale = locale;
    return this;
  }
  public setOptimizelyGraphClient(
    client: IOptiGraphClient | ((currentClient: IOptiGraphClient | undefined) => IOptiGraphClient | undefined)
  ) {
    if (this.isDebug) console.log(`🦺 [ServerContext] Assigning new Optimizely Graph Client`);
  ...
  public setComponentFactory(
    factory: ComponentFactory | ((currentClient: ComponentFactory | undefined) => ComponentFactory)
  ) {
    if (this.isDebug) console.log(`🦺 [ServerContext] Assigning new Component Factory`);
    const newFactory = typeof factory == "function" ? factory(this._factory) : factory;
    if (!newFactory) throw new Error("Unsetting the context factory is not allowed!");
    this._factory = newFactory;
  }
  public setEditableContentId(link: GenericContext["editableContent"]) {
    if (this.isDebug) console.log(`🦺 [ServerContext] Assigning editable content id: ${JSON.stringify(link)}`);
    this._editable = link;
  }
 ...
 */
export function updateSharedServerContext(currentCtx: GenericContext): ServerContext {
  ...
  else if (!currentCtx.inEditMode && !currentCtx.inPreviewMode) serverCtx.setMode("public");
