...
import { type GenericContext, type TransferrableContext } from "./types";
import {
  getFactory,
...
import { UndefinedComponentFactory } from "../factory/undef";
import { isTransferrableContext } from "./shared";
...
export class ClientContextInstance implements GenericContext {
  ...
  public constructor(cfg: TransferrableContext, components: ComponentTypeDictionary) {
...
export const OptimizelyCms: FunctionComponent<PropsWithChildren<OptimizelyCmsProps>> = ({
  ...
  const [editableContentIsExperience, setEditableContentIsExperience] = useState<boolean>(false);
  //#endregion
  ...
  const isDev = useMemo(
    () => (isDevelopment == undefined ? getNodeEnv() == "development" : isDevelopment),
    [isDevelopment]
  );
...
export function fromTransferrableContext(
  ...
  if (!isTransferrableContext(serialized)) throw new Error("Unable to restore from serialized data");
