...
import type { ContentLinkWithLocale, ContentLink } from "@niteco/optimizely-graph-client";
import type {
  ContentType,
  BaseCmsComponent,
  CmsComponentWithQuery,
  CmsComponentWithFragment,
  WithGqlFragment,
  ContentQueryProps,
} from "./types";
import type { ComponentFactory, ComponentTypeHandle } from "./factory/types";

import { TYPE_MERGE_SYMBOL } from "./factory/index";
import { localeToGraphLocale } from "@niteco/optimizely-graph-client/services/utils";

//Export the Rich-Text utilities
export * from "./components/rich-text/utils";
...
export function isContentType(toTest: any): toTest is ContentType {
  if (!Array.isArray(toTest)) return false;
...
export function normalizeContentType(
  toNormalize: (string | null)[] | string | null | undefined,
  stripContent: boolean = false
): ContentType | undefined {
  if (!toNormalize) return undefined;

  let filtered = (typeof toNormalize == "string" ? toNormalize.split("/") : toNormalize)
    .filter(isNonEmptyString)
    .map((x) => (x.startsWith("_") ? x.substring(1) : x));
  if (stripContent) filtered = filtered.filter((x) => x.toLowerCase() != "content");
 ...
 * @returns
 */
export function normalizeAndPrefixContentType(
  contentType: Array<string | null> | string | null | undefined,
  prefix: string
): ContentType {
  if (!contentType) return [prefix];

  const processedContentType = normalizeContentType(contentType, true) ?? [];
  if (processedContentType[0] != prefix) processedContentType.unshift(prefix);
...
): ReturnType<ComponentFactory["resolve"]> {
...
export function isCmsComponentWithDataQuery<T = DocumentNode>(
  toTest?: BaseCmsComponent<T>
): toTest is CmsComponentWithQuery<T> {
  const toTestType = typeof toTest;
  if ((toTestType == "function" || toTestType == "object") && toTest != null) {
    return (toTest as CmsComponentWithQuery).getDataQuery &&
      typeof (toTest as CmsComponentWithQuery).getDataQuery == "function"
      ? true
      : false;
  }
  return false;
...
export function isCmsComponentWithFragment<T = DocumentNode>(
  toTest?: BaseCmsComponent<T>
): toTest is CmsComponentWithFragment<T> {
  const toTestType = typeof toTest;
  if ((toTestType == "function" || toTestType == "object") && toTest != null)
    return (toTest as CmsComponentWithFragment).getDataFragment &&
      typeof (toTest as CmsComponentWithFragment).getDataFragment == "function"
      ? true
      : false;
  return false;
}

export function validatesFragment<T extends ComponentType<any>>(
  toTest?: T
): toTest is T & Pick<Required<WithGqlFragment<T, any>>, "validateFragment"> {
  type ToTestFor = T & Pick<Required<WithGqlFragment<T, any>>, "validateFragment">;
  const toTestType = typeof toTest;
  if ((toTestType == "function" || toTestType == "object") && toTest != null)
    return (toTest as ToTestFor).validateFragment && typeof (toTest as ToTestFor).validateFragment == "function"
      ? true
      : false;
  return false;
}
...
export function contentLinkToRequestVariables(contentLink: ContentLinkWithLocale): ContentQueryProps {
  ...
  if (variables.version == undefined || variables.version == "") variables.version = null;
  return variables;
}
...
export function trim<T extends string | null | undefined>(valueToTrim: T): T {
  if (typeof valueToTrim == "string") return valueToTrim.trim() as T;
  return valueToTrim;
}
