...
import type { ContentLinkWithLocale, ContentLink, InlineContentLinkWithLocale } from "@niteco/optimizely-graph-client";
import type { GenericContext } from "./context/types";

// Export reused content types
export type ContentType = string[];
export type {
  /**
   * @deprecated  Use the export from `@niteco/optimizely-graph-client` directly
   */
  ContentLink,
  /**
   * @deprecated  Use the export from `@niteco/optimizely-graph-client` directly
 ...
 */
export type ResponseDataType<T extends DocumentNode> =
  T extends TypedDocumentNode<infer DataType> ? DataType : { [key: string]: any };
...
export type BaseCmsComponent<T = {}, L extends Record<string, any> = Record<string, any>> = T extends
  | never
  | TypedDocumentNode
  | DocumentNode
...
export type DynamicCmsComponent<
  T extends TypedDocumentNode | DocumentNode = DocumentNode,
  L extends Record<string, any> = Record<string, any>,
> = ReactComponentType<CmsComponentProps<ResponseDataType<T>, L>>;
export type GraphQLFragmentBase = { " $fragmentName"?: string };
export type GraphQLQueryBase = { __typename?: "Query" };
export type CmsComponentWithFragment<
  T = DocumentNode,
  L extends Record<string, any> = Record<string, any>,
> = BaseCmsComponent<T, L> & WithGqlFragment<{}, T>;
export type CmsComponentWithQuery<
  T = DocumentNode,
  L extends Record<string, any> = Record<string, any>,
> = BaseCmsComponent<T, L> & WithGqlQuery<{}, T>;
export type CmsComponentWithOptionalQuery<
  T = DocumentNode,
  L extends Record<string, any> = Record<string, any>,
> = BaseCmsComponent<T, L> & Partial<WithGqlQuery<{}, T>>;
...
export type CmsComponent<T = DocumentNode, L extends Record<string, any> = Record<string, any>> =
  T extends TypedDocumentNode<infer R, any>
    ? CmsComponentWithQuery<R, L>
    : T extends DocumentNode
      ? CmsComponentWithQuery<{ [key: string]: any }, L>
      : T extends GraphQLFragmentBase
        ? CmsComponentWithFragment<T, L>
        : T extends GraphQLQueryBase
          ? CmsComponentWithQuery<T, L>
          : CmsComponentWithOptionalQuery<T, L>;
export type CmsLayoutComponent<L extends Record<string, any> = Record<string, any>, T = never> = ReactComponentType<
  CmsComponentProps<T, L>
>;
